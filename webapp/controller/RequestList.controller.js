...
sap.ui.define([
	...
	"sap/ui/model/Sorter",
], function (BaseController, SharedData, JSONModel, History, formatter, Filter, FilterOperator, Sorter) {
	"use strict";
	var filtername;
	return BaseController.extend("com.bmc.hcm.drf.zhcmuxdrf.controller.RequestList", {
		...
		onDisplayForm: function (oEvent) {
			try {
				...
				var oFormData = this.getModel().getProperty(oSource.getParent().getParent().getBindingContextPath());
		...
		_openOrClose: function (sErfid, sErfst) {
			...
			oModel.callFunction("/SetOpenOrClose", {
				...
				success: function (oData, oResponse) {
					...
					that._callMessageToast(that.getText("APPROVED_REQUESTS_ME_SUCCES"), "S");
				},
				error: function (oError) {
					that._closeBusyFragment();
					that._callMessageToast(that.getText("APPROVED_REQUESTS_ME_ERROR"), "E");
				}
			});
